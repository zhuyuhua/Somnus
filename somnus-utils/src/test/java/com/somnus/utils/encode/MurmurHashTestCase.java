 ...
 */
package com.somnus.utils.encode;
...
import com.somnus.utils.bytecode.ByteUtil;
...
public class MurmurHashTestCase {
	...
	private static Logger logger = LoggerFactory.getLogger(MurmurHashTestCase.class);
	...
	public void testHash64StringCPP() {
		...
		logger.debug("Unsigned : " + unsigned);

		logger.debug("Unsigned > Long.MAX_VALUE ? "
				+ ((unsigned.compareTo(BigInteger.valueOf(Long.MAX_VALUE)) == 1) ? "true" : "false"));
		logger.debug("Signed == -1 ? " + (signed.compareTo(BigInteger.valueOf(-1)) == 0 ? "true" : "false"));

		logger.debug(hash + "");

		logger.debug(ByteUtil.longToByte(hash) + "");
