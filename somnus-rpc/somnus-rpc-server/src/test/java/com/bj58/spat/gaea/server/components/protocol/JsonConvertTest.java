...
public class JsonConvertTest extends TestCase {
	...
	public void testConvertToCharacter() throws Exception {
		for (int i = 0; i < 1; i++) {
			String jsonStr = JsonHelper.toJsonExt(new Character('a'));
		...
		Character actual = new JsonConvert().convertToCharacter(jsonObj.get("fCharacter"));
	...
	public void testConvertToString() throws Exception {
		...
		String actual = new JsonConvert().convertToString(jsonObj.get("fString"));
	...
	public void testConvertToboolean() throws Exception {
		...
		boolean actual = new JsonConvert().convertToboolean(jsonObj.get("fboolean"));
	...
	public void testConvertTodouble() throws Exception {
		...
		double actual = new JsonConvert().convertTodouble(jsonObj.get("fdouble"));
	...
	public void testConvertToT() throws Exception {
		...
		TestEntityII actual = (TestEntityII) new JsonConvert().convertToT(jsonObj.get("fObject"), TestEntityII.class);

		junit.framework.Assert.assertEquals(((TestEntityII) te.getFObject()).getId(), actual.getId());
		junit.framework.Assert.assertEquals(((TestEntityII) te.getFObject()).getTitle(), actual.getTitle());
	}
	...
	public void testJSON() throws Exception {
		...
		Map<String, String> mapII = (Map<String, String>) JsonHelper.toJava(jsonStr, Map.class);
		System.out.println(mapII.get("k1"));
		System.out.println(mapII.get("k2"));
		System.out.println(mapII.get("k3"));

		TestEntityII[] teAry = new TestEntityII[] { new TestEntityII(1, "title1"), new TestEntityII(2, "title2") };
		jsonStr = JsonHelper.toJsonExt(teAry);
		System.out.println(jsonStr);

		teAry = (TestEntityII[]) JsonHelper.toJava(jsonStr, TestEntityII[].class);
