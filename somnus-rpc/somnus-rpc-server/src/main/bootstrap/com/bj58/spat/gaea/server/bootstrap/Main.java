...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		args = new String[] { "-Dgaea.service.name=demo" };
		if (args.length < 1) {
		...
		classLoader.addFolder(rootPath + "service/deploy/" + sc.getString("gaea.service.name") + "/",
				rootPath + "service/lib/", rootPath + "lib");

		GlobalClassLoader.addSystemClassPathFolder(
				rootPath + "service/deploy/" + sc.getString("gaea.service.name") + "/", rootPath + "service/lib/",
				rootPath + "lib");
		logger.info("-------------------------end-------------------------\n");
