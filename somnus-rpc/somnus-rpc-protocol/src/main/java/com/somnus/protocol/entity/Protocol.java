...
import com.somnus.protocol.compress.CompressBase;
...
import com.somnus.protocol.secure.DESCoderHelper;
import com.somnus.protocol.serializer.SerializeBase;
import com.somnus.utils.bytecode.ByteConverter;
...
public class Protocol {
	...
	private SerializeType serializeType = SerializeType.SomnusBinary;
	...
	public Protocol(int sessionId, byte serviceId, SDPType sdpType,
			CompressType compressType, SerializeType serializeType,
	...
	public byte[] toBytes() throws Exception {
		int startIndex = 0;
		SerializeBase serialize = SerializeBase.getInstance(this
				.getSerializeType());
		CompressBase compress = CompressBase
				.getInstance(this.getCompressType());
		...
		System.arraycopy(
				ByteConverter.intToBytesLittleEndian(this.getTotalLen()), 0,
				data, startIndex, SFPStruct.TotalLen);

		startIndex += SFPStruct.TotalLen;
		System.arraycopy(
				ByteConverter.intToBytesLittleEndian(this.getSessionID()), 0,
				data, startIndex, SFPStruct.SessionId);
	...
	public static Protocol fromBytes(byte[] data) throws Exception {
		...
		p.setSdpEntity(serialize.deserialize(sdpData,
				SDPType.getSDPClass(p.getSDPType())));
		return p;
	}
	 ...
	 * @throws Exception
	 */
	public static Protocol fromBytes(byte[] data, boolean rights, byte[] desKey)
			throws Exception {
		Protocol p = new Protocol();
		int startIndex = 0;
		if (data[startIndex] != Protocol.VERSION) {
